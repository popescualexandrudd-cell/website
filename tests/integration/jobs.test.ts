...
describe("background jobs", () => {
  it("sends the 24-hour reminder once, only for lessons booked well in advance", async () => {
    ...
    const late = await booking({
      startsAt: new Date(Date.now() + 22 * HOUR),
      createdAt: new Date(Date.now() - HOUR),
    });
    expect(await sendDueReminders()).toBe(1);
    expect(await sendDueReminders()).toBe(0);
    expect(
      await db.emailLog.count({ where: { template: "booking-reminder", bookingId: early.id } }),
    ).toBe(1);
    expect(await db.emailLog.count({ where: { bookingId: late.id } })).toBe(0);
    expect(
      (await db.booking.findUniqueOrThrow({ where: { id: late.id } })).reminderSentAt,
    ).not.toBeNull();
  });
  ...
  it("invites a review only after the client's first completed lesson", async () => {
    ...
    expect(
      (await db.booking.findUniqueOrThrow({ where: { id: second.id } })).reviewTokenHash,
    ).toBeNull();
  });
  ...
  it("anonymises personal data past the retention period and keeps the statistics", async () => {
    const old = await booking({
      status: "EFECTUATA",
      startsAt: new Date(Date.now() - 800 * 24 * HOUR),
    });
    ...
    expect((await db.booking.findUniqueOrThrow({ where: { id: recent.id } })).name).toBe(
      "Ion Popescu",
    );
  ...
  it("cleans expired sessions and rate-limit windows", async () => {
    await db.rateLimit.create({
      data: { key: "old", count: 1, windowStart: new Date(0), expiresAt: new Date(1000) },
    });
