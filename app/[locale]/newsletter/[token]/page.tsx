...
export default async function NewsletterPage({ params }: PageProps<"/[locale]/newsletter/[token]">) {
  const { locale: raw, token } = await params;
  const locale = raw as Locale;
  setRequestLocale(locale);
  const t = await getTranslations("footer");
  const unsubscribe = token.startsWith("u.");
  const title = unsubscribe ? (locale === "en" ? "Unsubscribe" : "Dezabonare") : t("newsletterTitle");
  const label = unsubscribe ? (locale === "en" ? "Unsubscribe me" : "Dezabonează-mă") : locale === "en" ? "Confirm subscription" : "Confirmă abonarea";
  return (
