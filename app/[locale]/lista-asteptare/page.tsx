...
export async function generateMetadata({ params }: PageProps<"/[locale]/lista-asteptare">): Promise<Metadata> {
  const locale = (await params).locale as Locale;
  const header = await getPageHeader("lista-asteptare", locale);
  return pageMetadata({ locale, href: "/lista-asteptare", title: header.seoTitle, description: header.seoDescription });
}

export default async function WaitlistPage({ params, searchParams }: PageProps<"/[locale]/lista-asteptare">) {
  const locale = (await params).locale as Locale;
  setRequestLocale(locale);
  const query = await searchParams;
  const [header, programs, h] = await Promise.all([getPageHeader("lista-asteptare", locale), getPrograms(locale), headers()]);
