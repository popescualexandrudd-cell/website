...
type Props = { programs: { id: string; name: string; forMinors: boolean }[]; initialProgramId?: string | null; turnstileSiteKey?: string | null; nonce?: string };

export function WaitlistForm({ programs, initialProgramId, turnstileSiteKey, nonce }: Props) {
  const t = useTranslations();
  const locale = useLocale();
  const errorText = useErrorText();
  const [state, action] = useActionState(submitWaitlist, idleState);
  if (state.status === "success") return <FormStatus state={state} success={t("waitlist.success")} />;
  return (
    <form action={action} className="grid max-w-2xl gap-5" noValidate>
      <input type="hidden" name="locale" value={locale} />
      <Honeypot />
      <SelectField name="programId" label={t("waitlist.program")} optional defaultValue={initialProgramId ?? ""}>
      ...
      <div className="booking-fields">
        <TextField name="name" autoComplete="name" label={t("contact.name")} error={errorText(state.fieldErrors?.name)} />
        <TextField name="email" type="email" autoComplete="email" label={t("contact.email")} error={errorText(state.fieldErrors?.email)} />
        <TextField name="phone" type="tel" autoComplete="tel" label={t("booking.details.phone")} error={errorText(state.fieldErrors?.phone)} />
        <TextField
          name="childAge"
          type="number"
          inputMode="numeric"
      ...
      <div>
        <SubmitButton>{t("waitlist.submit")}</SubmitButton>
      </div>
    </form>
  );
}
